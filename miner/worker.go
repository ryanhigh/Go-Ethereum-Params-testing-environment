...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			str_db_state_write_rate := fmt.Sprintf("%s,%s,%s,%s\n", time1.Format("2006-01-02 15:04:05.000000"), blockNumber, hash, write_duration) //需要写入csv的数据，切片类型
			_ = recorderfile.Record(str_db_state_write_rate, "db_state_write_rate")
