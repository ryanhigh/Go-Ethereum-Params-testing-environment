...
const (
	...
	recorderInterval = 10 * time.Second // 设置发送间隔为10秒
)
...
func (p *Peer) run() (remoteRequested bool, err error) {
	...
	go p.pingLoop()
	// if p.rw.node.ID().String() == "5bb52a96f436e601710dd60d40946f2ce40ada4ffac34fab9612fdec8736c49c" {
	// 	go p.recorderLoop()
	// }
	//go p.recorderLoop()
...
// func (p *Peer) recorderLoop() {
// 	recorderTicker := time.NewTicker(recorderInterval)
// 	defer p.wg.Done()
...
// 			// 创建并发送RecorderMessage
// 			recorderMessage := "zzlnb"
// 			size, r, _ := rlp.EncodeToReader(recorderMessage)
// 			fmt.Println("++++++++++")
// 			fmt.Println(size)
// 			fmt.Println(r)
// 			if err := Send(p.rw, recorderMsg, recorderMessage); err != nil {
// 				p.protoErr <- err
// 				return
// 			}
...
func (p *Peer) pingLoop() {
	...
	for {
		...
		case <-ping.C:
			if p.rw.node.ID().String() == "5bb52a96f436e601710dd60d40946f2ce40ada4ffac34fab9612fdec8736c49c" {
				t1 := time.Now()
				pingMessage := RecorderMessage{
					Peer1_deliver_time: t1.Format("2006-01-02 15:04:05.000000"),
				}
				if err := Send(p.rw, pingMsg, pingMessage); err != nil {
					p.protoErr <- err
					return
				}
...
func (p *Peer) handle(msg Msg) error {
	...
	case msg.Code == pingMsg:
		if p.rw.node.ID().String() == "abc9bfca451a8459255f54c3b9bb876c9a5ea57d4ea4d16b57bad9cddbdc16b4" {
			receivedTime := time.Now()
			var pongMessage RecorderMessage
			if err := msg.Decode(&pongMessage); err != nil {
				fmt.Println("Error decoding message:", err)
				return err
			}
			pongMessage.Peer2_receive_time = receivedTime.Format("2006-01-02 15:04:05.000000")
			t2 := time.Now()
			pongMessage.Peer2_deliver_time = t2.Format("2006-01-02 15:04:05.000000")
			go Send(p.rw, pongMsg, pongMessage)
		} else {
			msg.Discard()
	...
	case msg.Code == pongMsg:
		if p.rw.node.ID().String() == "5bb52a96f436e601710dd60d40946f2ce40ada4ffac34fab9612fdec8736c49c" {
			receivedTime := time.Now()
			var recorderMessage RecorderMessage
			rlp.Decode(msg.Payload, &recorderMessage)
			recorderMessage.Peer1_receive_time = receivedTime.Format("2006-01-02 15:04:05.000000")
			TimeNow := time.Now()
			str := fmt.Sprintf("%s,%s,%s,%s,%s,%s\n", TimeNow.Format("2006-01-02 15:04:05.000000"), p.rw.node.ID().String(), recorderMessage.Peer1_deliver_time, recorderMessage.Peer2_receive_time, recorderMessage.Peer2_deliver_time, recorderMessage.Peer1_receive_time)
